...
export default function Chats({
  ...
  const normalClassName =
    "grid grid-rows-[auto auto] rounded-lg border px-5 relative overflow-y-auto overscroll-y-scroll pt-5 w-full h-full max-h-1/2 shadow-inner  ";
  const loadingClassName =
    "grid grid-rows-[auto auto] px-5  left-0 overflow-y-auto overscroll-y-auto w-full max-h-full bg-[#fff]";
  return (
    <>
      <div className={isMessageLoading ? loadingClassName : normalClassName}>
        {isMessageLoading ? (
          <div className="text-black flex flex-col gap-3 items-center justify-center mt-2">
            {" "}
...
function HumanMessage({ message }: { message: string }) {
  ...
  return (
    <div className="p-4 flex items-start justify-end space-x-4 w-full ">
      <div className="grid grid-rows-auto gap-0.5 ">
        <div className="rounded-lg bg-gradient-to-b from-[#e0d3cc] to-[#977364cf] dark:bg-gray-800 p-4 text-sm border border-gray-900">
          {message}
        </div>
      </div>
      <div>
